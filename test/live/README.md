...
| `test_dependencies.tf` | A dedicated, throwaway resource group and virtual network this harness owns outright - never a shared/production resource group. Its name is suffixed with `var.pr_number` so concurrently open PRs never collide. |
| `variables.tf` | `env`, `location` (defaults to `canadacentral`), `tags`, `pr_number` (defaults to `"manual"`), `zone_name` (the DNS zone FQDN), and `private_dns_zone` (typed `any`, passed straight through to the module). |
| `config/private_dns_zone.tfvars` | One representative real-usage fixture: a single private DNS zone linked to a dedicated vnet via the "DNS" resource-group key. |
...
cd test/live
terraform init
terraform plan  -var-file=config/private_dns_zone.tfvars
terraform apply -var-file=config/private_dns_zone.tfvars
```

Confirm only the live-test resource group, vnet, and `module.private_dns_zone`
are planned/applied, then tear it down:

```bash
terraform destroy -var-file=config/private_dns_zone.tfvars
```
...
terraform apply -var-file=config/private_dns_zone.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/private_dns_zone.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/private_dns_zone.tfvars -var="pr_number=<pr-number>"
```
